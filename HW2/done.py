...
def compute_distances_two_loops(x_train, x_test):
  ...
  x_train_flat=torch.reshape(x_train,(num_train,-1))
  x_test_flat=torch.reshape(x_test,(num_test,-1))
  for trn in range(num_train):
	  for tst in range(num_test):
  		dists[trn,tst]=torch.sum((x_test_flat[tst]-x_train_flat[trn])**2)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_one_loop(x_train, x_test):
  ...
  x_test_flat=torch.reshape(x_test,(num_test,-1))
  for trn in range(num_train):
  	dists[trn]=torch.sum((x_train_flat[trn]-x_test_flat)**2,dim=1)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_no_loops(x_train, x_test):
  ...
  x_train_flat=torch.reshape(x_train,(num_train,-1))
  x_test_flat=torch.reshape(x_test,(num_test,-1))
  x_trn_pow2_sum=torch.sum(x_train_flat**2,dim=1,keepdim=True)
  x_tst_pow2_sum=torch.sum(x_test_flat**2,dim=1,keepdim=True).t()
  ab=torch.mm(x_train_flat,x_test_flat.t())
  dists=x_trn_pow2_sum+x_tst_pow2_sum-2*ab
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def predict_labels(dists, y_train, k=1):
  ...
  # Replace "pass" statement with your code
  for tst in range(num_test):
     _, idx = torch.topk(-dists[:,tst],k)
     y_pred[tst],_=torch.mode(y_train[idx],0)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return y_pred
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices:
    ...
    for i in range(num_folds):
      ...
      k_to_accuracies[k].append(knn.check_accuracy(x_val_fold,y_val_fold,k))
  ##############################################################################
  #                            END OF YOUR CODE                                #
  ##############################################################################
...
def knn_get_best_k(k_to_accuracies):
  ...
  for k in k_to_accuracies:
    mean_dict[k]=sum(k_to_accuracies[k])/len(k_to_accuracies[k])
