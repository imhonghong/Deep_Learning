...
def svm_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  learning_rates = [3.5e-3,1e-2,3.5e-2,1e-3]
  regularization_strengths = [3.5e-4,1e-4,3.5e-3,1e-3,3.5e-2,1e-2]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
...
def test_one_param_set(cls, data_dict, lr, reg, num_iters=2000):
  ...
  train_acc = torch.sum(train_pred == data_dict['y_train']) / data_dict['y_train'].shape[-1]
  test_pred=cls.predict(data_dict['X_val'])
  val_acc = torch.sum(test_pred == data_dict['y_val']) / data_dict['y_val'].shape[-1]
  ############################################################################
  #                            END OF YOUR CODE                              #
  ############################################################################
...
def softmax_loss_naive(W, X, y, reg):
  ...
  for i in range(num_train):
    ...
    loss += -torch.log(exp_correct_class_score / torch.sum(exp_scores))
    for j in range(num_classes):
      if (j==y[i]):
        dW[:, y[i]] += (-1 + exp_scores[j] / torch.sum(exp_scores)) * X[i,:]
      else:
        dW[:, j] += (exp_scores[j] / torch.sum(exp_scores)) * X[i,:]
  loss /= num_train
  loss += reg * torch.sum(W * W)
  dW /= num_train
  dW += 2 * reg * W
...
def softmax_loss_vectorized(W, X, y, reg):
  ...
  num_classes = W.shape[1]
  scores = X.mm(W).t()
  max_s, _ = torch.max(scores, dim=0)
  scores -= max_s
  exp_scores = torch.exp(scores)
  exp_correct_class_score = exp_scores[y, torch.arange(num_train)]
  loss = -torch.log(exp_correct_class_score / torch.sum(exp_scores, dim=0)).sum()
  loss /= num_train
  loss += reg * torch.sum(W * W)
  A=exp_scores / torch.sum(exp_scores, dim=0)
  A[y, torch.arange(num_train)] -= 1
  dW = X.t().mm(A.t())
  dW /= num_train
  dW += 2 * reg * W
...
def softmax_get_search_params():
  ...
  ###########################################################################
  # Replace "pass" statement with your code
  learning_rates = [1.9e-2,3.5e-2,5.6e-2,1e-1]
  regularization_strengths = [1e-4,3.5e-4,1e-3,1.7e-3,3.5e-3]
  ###########################################################################
  #                           END OF YOUR CODE                              #
  ###########################################################################
